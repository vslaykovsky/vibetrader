...
`params-hyperopt.json` may set `mode: "walk_forward"` with a `walk_forward` object containing `train_window_days`, `test_window_days`, and `oos_total_days`. `test_window_days` is both the OOS fold length and retraining interval, so OOS execution remains contiguous. These are study settings, not strategy tunables. For US equities, an OOS fold containing no NYSE session (weekends and exchange holidays only) is recorded as skipped and consumes no optimization trials; carried OOS portfolio state passes unchanged to the next active fold. In walk-forward mode, each active fold optimizes on the train window, runs the selected params over train+OOS for strategy warmup, crops rendered output and metrics to the OOS segment, stitches OOS charts/orders/equity into `backtest.json`, writes aggregate metrics to `metrics.json`, writes fold diagnostics to `walkforward.json`, and restores the original `params.json`.

Keep the default `sampler: "bayesian"` unless the user explicitly asks for grid, exhaustive, or grid-like optimization. Rust grid studies use `sampler: "grid"`; every active numeric search-space spec must provide a positive `step` (integer steps default to `1`, but still prefer an explicit value), while categorical `choices` are already discrete. `n_trials` caps the deterministic Cartesian grid, so set it to the full combination count for an exhaustive grid. Grid sampling is Rust-only; never silently run the default sampler for an explicit grid request.

...
Optimization defaults to `sampler: "bayesian"`. Only explicit user requests for grid, exhaustive, or grid-like optimization should set `sampler: "grid"`. In grid mode, numeric specs use positive `step` values (integer specs default to `1`; float specs require `step`), categorical `choices` define their discrete values, and `n_trials` caps the deterministic Cartesian product. An explicit grid request must fail clearly if the native Rust optimizer cannot support the strategy topology; it must not fall back to the default sampler.

...
backend/strategies - Per-thread strategy workspaces at `backend/strategies/<THREAD_UUID>/` (seeded from `backend/strategies/AGENTS.md`). The backend agent runs Codex on the first codegen in a thread, then Claude Code (`claude code --prompt ... --yes --quiet`) on later updates, and backtests in that folder using `python src/strategy.py` with cwd set to the thread directory.
